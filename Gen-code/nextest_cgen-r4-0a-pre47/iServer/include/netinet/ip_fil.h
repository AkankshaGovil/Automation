 ...
 * $Id: ip_fil.h,v 1.1.1.1.2.3 2004/05/05 17:52:36 src Exp $
 */
...
# define	SIOCINAFR_N		_IOW('r', 102, struct frentry64 )
# define	SIOCADDPME		_IOW('r', 103, struct fr_pm_entry )
# define	SIOCRMPME		_IOW('r', 104, struct fr_pm_entry )
# define	SIOCCLRBM		_IOW('r', 105, struct fr_clearbm_entry )
# define	SIOCSETITYPE	_IOW('r', 106, struct fr_interface_type )
# define	SIOCSETIVIP		_IOW('r', 107, struct fr_interface_vip )
#else
# define	SIOCADAFR		_IOW(r, 60, struct frentry *)
# define	SIOCRMAFR		_IOW(r, 61, struct frentry *)
# define	SIOCSETFF		_IOW(r, 62, u_int)
...
typedef	struct	fr_ip
{
	...
	union i6addr	fi_src;		/* src  addr from packet (network order) */
	union i6addr	fi_dst;		/* dest addr from packet (network order) */
...
{
	void    *		fin_ifp;		/* ill_t* for interface packet is `on' */

    struct  fr_ip   fin_fi;			/* IP Packet summary */

    u_short 		fin_data[2];	/* TCP/UDP ports (host order)  */
									/* fin_data[0] is src port     */
									/* fin_data[1] is dest port    */
									/* or ICMP code/type           */
	...
	struct frentry*	fin_fr;			/* pointer to last matching rule */

	char *			fin_dp;			/* pointer to start of payload */
									/* past ip header              */

	u_short			fin_plen;		/* total length of packet */

	u_short			fin_off;		/* fragmentation offset of packet from
									** ip header ( generally 0 since most
									** of the traffic passing through us
									** is not fragmented
									*/
...
typedef	struct	frentry
{
	...
	struct fr_ip		fr_ip;
	struct fr_ip		fr_mip;		// mask structure for fields in fr_ip
...
typedef	struct	frentry64
{
	...
	u_int				fr_loglevel;// syslog log facility + priority
	u_int				unused1;
	char				fr_func[8];	// function pointer to function to be called
...
typedef	struct	fr_interface_vip
{
	char			ifname[LIFNAMSIZ];
	uint32_t		vip;
} fr_interface_vip_t;
...
extern	int	ipfr_fastroute_mr(	ip_t *			ip,
								mblk_t *		ih_mb_ptr,
								mblk_t *		ih_prev_mb_ptr,
								mblk_t **		first_mb_pptr,
				    			fr_info_t *		fin,
								void *			fastr_info,
								void *			ipnat_ptr );

extern	qif_t *	qif_from_ill( ill_t * ill );
...
extern	int	new_ircopy __P((void *, void *, size_t, int ));
