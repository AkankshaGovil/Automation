 ...
 * $Id: ip_compat.h,v 1.1.1.1.2.3 2004/05/05 17:52:36 src Exp $
 */
 ...
 */
	...
	typedef	struct	qif
	{
		...
		struct	qinit *	qf_wqinfo;	// saved pointer to original qinit struct
									// for write side. used to remove ipf
									// at detach time.

		struct	qinit *	qf_rqinfo;	// saved pointer to original qinit struct
									// for read side. used to remove ipf
									// at detach time.

		struct	qinit	qf_wqinit;	// our copy of qinit structure with put
									// procedure modified to fr_qout()

		struct	qinit	qf_rqinit;	// our copy of qinit structure with put
									// procedure modified to fr_qin()

		// The following three fields are for passing 
		// data up from fr_qin and fr_qout to the packet
		// processing.
		...
		// the following field is used for in ipfr_fastroute
		...
		uint32_t		qf_ip4_lcl;		// ip4 local addr info for interface
		uint32_t		qf_itype;		// interface type
		uint32_t		qf_ip4_vip;		// ip4 local addr info for vip
										// in redundant configuration
										// interface

		uint16_t		qf_largest_anon_udp_port;
