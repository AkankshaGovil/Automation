...
int tst_insert(unsigned char *key, void *data, struct tst *tst, int option, void **exist_ptr)
{
   ...
   while(perform_loop == 1)
   {
      if(toascii(key[key_index]) == current_node->value)
      {
         ...
         else
         {
            if(current_node->middle == NULL)
            {
               ...
               current_node->value = toascii(key[key_index]);
               break;
            }
            else
            {
      ...
      if( ((current_node->value == 0) && (toascii(key[key_index]) < 64)) ||
