...
static inline void l_enqueue(spthread_descr * q, spthread_descr th)
{
...
static inline spthread_descr l_dequeue(spthread_descr * q)
{
  spthread_descr th;
  th = *q;
  if (th != NULL) {
...
static inline void l_remove_from_queue(spthread_descr * q, spthread_descr th)
{
  for (; *q != NULL; q = &((*q)->p_nextwaiting)) {
    if (*q == th) {
      *q = th->p_nextwaiting;
