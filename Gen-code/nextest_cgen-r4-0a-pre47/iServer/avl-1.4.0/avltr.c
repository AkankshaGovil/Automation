...
void *(*_avltr_malloc)(size_t) = SHM_Malloc;
void (*_avltr_free)(void *) = SHM_Free;
void *(*_avltr_realloc)(void *, size_t) = SHM_Realloc;

#define malloc(n)	_avltr_malloc(n)
#define free(ptr)	_avltr_free(ptr)
#define realloc(ptr, size)	_avltr_realloc(ptr, size)

int
avltr_init(int mtype)
{
	if (mtype)
	{
		_avltr_malloc = malloc;
		_avltr_free = free;
		_avltr_realloc = realloc;
	}
	else
	{
		_avltr_malloc = SHM_Malloc;
		_avltr_free = SHM_Free;
		_avltr_realloc = SHM_Realloc;
	}
}
...
avltr_tree *
avltr_create (int cmp, int inscmp, void *param)
{
  avltr_tree *tree;
...
avltr_copy (const avltr_tree *tree, avl_copy_func copy)
{
  ...
  assert (tree != NULL);
  new_tree = avltr_create (tree->cmp, tree->inscmp, tree->param);
