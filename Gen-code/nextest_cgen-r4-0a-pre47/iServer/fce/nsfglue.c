...
nsfGlueInit(void)
{
	...
	if ( (num = getPooledInterfaces( POOL_INTF_SIG, signaling_ifs) ) ) {
		...
		for (i = 0; i < num; i++) {
			...
			if ( ioctl( ipf_fd, SIOCSETITYPE, &fr_it ) < 0 ) {
				NETERROR(	MFCE,
							("Error setting interface type for signaling "
							"interface %s\n",
							signaling_ifs[i] ));
				continue;
			}
			...
			if (ioctl(ipf_fd, SIOCCLRBM, &fr_cb) < 0) {
				NETERROR(	MFCE,
							("Error clearing bitmap for signaling"
							 " interface, %s\n",
							signaling_ifs[i]) );
				continue;
			}

			// Builds the fwconfig command for a firewalled
			// configuration
			//     Format example :
			//        fwconfig -s e1000g0,e1000g1
		...
		// Builds the fwconfig command for an open
		// configuration
		//     Format example :
		//        fwconfig -d
	...
	// Issue fwconfig command with built arguments
	...
	if( !IS_HKNIFE_FW ) {
		// Clear any nat redirect entries in kernel
	...
	if( !IS_HKNIFE_FW ) {
		for (i = 0; i < num; i++) {
			// set type for each media interface

			memset( &fr_it, 0, sizeof( fr_interface_type_t ) );
			strcpy( fr_it.ifname, media_ifs[i] );
			...
			{
				NETERROR( MFCE,
				          ("Error setting interface type for media "
				          "route interface %s\n",
				          media_ifs[i] ));
			}
		}
	} else {
		// TODO : add stuff for hotknife
	}
				...
				)
{
	...
	if ( ! IS_HKNIFE_FW ) {
		if (rtcp_nat_p->rdr_defined == TRUE)
		{
			// Remove rtcp nat translation

			RemoveRedirect(	bundleId,
						holeId,
						(void *) &rtcp_nat_p->data,
						rtcp_nat_p->rdr_string_1,
						rtcp_nat_p->rdr_string_2,
						rtcp_nat_p->rdr_string_3,
						"RTCP",
            ingress_vlan,
						nop );
				...
				uint32_t	peerResourceId )
{
	...
	if ( ! IS_HKNIFE_FW ) {
		...
		if (AddRedirect(	ingressInterface,
							...
							1,
							nop, 
							((other_hp)?&other_hp->rtcp_nat:NULL),
							other_hp,
							dstSym,
							peerResourceId,
              ingressVlanId,
		...
		{
			trc_error(	MFCE,
						"       : failed to add rtcp Redirect for hole %d\n",
						holeId );
			return -1;
		}
				...
				)
{
  ...
  int ingressVlanId, egressVlanId;
	...
	IPNAT_T			*nat_rtp_p;
	IpHole_t		*idp, *hp, *returned_hole_ptr;
	IPNAT_T			*nat_rtcp_p;
	...
	if (modified) {
		if( !IS_HKNIFE_FW ) {
			UnplumbMediaHole(idp->bundleId,	idp->assignedId, &idp->rtp_nat, &idp->rtcp_nat,
                       ingressVlanId, idp->flags & HOLE_ALLOCATE);
		}
		...
		if( !IS_HKNIFE_FW ) {
			...
			// Update/Add the hole in the cache
...
CloseHole(	IpHole_t * idp )
{
	...
	if ( ! IS_HKNIFE_FW ) {
		if (idp->rtcp_nat.rdr_defined == TRUE)
		{
			...
			idp->rtcp_nat.rdr_defined = FALSE;
		}
	}
...
static int
OpenDevice(char *devname, int *fdp)
{
        ...
        int egressVlanId )
{
	...
	DNAT_RELATE_T		relate;
	DNAT_RELATE_T *		p_relate = &relate;
	...
	if( IS_HKNIFE_FW ) {
		...
		if(relate_leg1_leg2) {
			memcpy(&relate.data_nat_key, &nat_rdr_ptr->in_legkey, sizeof(LEG_KEY_T));
			memcpy(&relate.other_key, &other_nat_rdr_ptr->in_legkey, sizeof(LEG_KEY_T));

			if(ioctl(ipnat_fd, SIOCRELATEDN, &p_relate) < 0) {
				int lerrno = errno;
				NETERROR(MFCE, ("AddRedirect(): relatedn leg_1_2 failed - hole ids "
								"%d %d keys %u 0x%8x %u 0x%8x error code %d\n",
								holeId,
								peerResourceId,
								relate.data_nat_key.hv,
								(uint32_t) relate.data_nat_key.ipnat_ptr,
								relate.other_key.hv,
								(uint32_t) relate.other_key.ipnat_ptr,
								lerrno) );
				trc_error(	MFCE, 
							"       : ERROR relatedn leg_1_2 failed - hole ids "
							"%d %d keys %u 0x%8x %u 0x%8x error code %d\n",
							holeId,
							peerResourceId,
							relate.data_nat_key.hv,
		...
		if(relate_leg2_leg1) {
			memcpy(&relate.data_nat_key, &other_nat_rdr_ptr->in_legkey, sizeof(LEG_KEY_T));
			memcpy(&relate.other_key, &nat_rdr_ptr->in_legkey, sizeof(LEG_KEY_T));
			...
			if(ioctl(ipnat_fd, SIOCRELATEDN, &p_relate) < 0) {
				...
				NETERROR(MFCE, ("AddRedirect(): relatedn leg_2_1 failed - hole ids "
								...
								lerrno) );
				trc_error(	MFCE, 
							"       : ERROR relatedn leg_2_1 failed - hole ids "
							"%d %d keys %u 0x%8x %u 0x%8x error code %d\n",
							holeId,
							peerResourceId,
							relate.data_nat_key.hv,
	...
	if ( IS_HKNIFE_FW ) {
		...
		trc_debug( MFCE,
                   NETLOG_DEBUG2,
				   "       :   returned legkey: hv %d, redir_addr 0x%08x\n",
				   nat_rdr_ptr->in_legkey.hv, nat_rdr_ptr->in_legkey.ipnat_ptr );
	}
	...
	if(relate_leg1_leg2) {
		if ( IS_HKNIFE_FW ) {
			...
			trc_debug(	MFCE, 
						NETLOG_DEBUG2,
						"       :   ioctl() - relate_dnat leg_1_2 for hid %d, phid %d\n",
						holeId,
						peerResourceId );
			trc_debug(	MFCE,
						NETLOG_DEBUG2,
						"             dnat leg  key %u 0x%8x\n",
				        nat_rdr_ptr->in_legkey.hv,
						(uint32_t) nat_rdr_ptr->in_legkey.ipnat_ptr );
			trc_debug(	MFCE,
						NETLOG_DEBUG2,
						"             other leg key %u 0x%8x\n",
				        other_nat_rdr_ptr->in_legkey.hv,
						(uint32_t) other_nat_rdr_ptr->in_legkey.ipnat_ptr );
		}
	}
	...
	if(relate_leg2_leg1) {
		if ( IS_HKNIFE_FW ) {
			...
			trc_debug(	MFCE, 
						NETLOG_DEBUG2,
						"       :   ioctl() - relate_dnat leg_2_1 for hid %d, phid %d\n",
						peerResourceId,
						holeId );
			trc_debug(	MFCE,
						NETLOG_DEBUG2,
	...
	if ( IS_HKNIFE_FW ) {
		...
		NETDEBUG(MFCE, NETLOG_DEBUG4, ("key returned %u 0x%8x", nat_rdr_ptr->in_legkey.hv, (uint32_t)nat_rdr_ptr->in_legkey.ipnat_ptr));
	}
