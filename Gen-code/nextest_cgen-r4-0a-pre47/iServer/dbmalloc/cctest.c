...
/*
 * $Id: cctest.c,v 1.1 2002/07/04 12:53:10 agoyal Exp $
 */
/*
...
#if __cplusplus
MEMDATA *
memccpy(MEMDATA * ptr1, CONST MEMDATA * ptr2, int ch, MEMSIZE len)
#else
MEMDATA *
...
#if __cplusplus
int
write(int fd, CONST char *buf, WRTSIZE size)
#else
int
write(fd, buf, size)
	int fd;
...
#endif
{
	char *bufptr = (void*) buf;

	if ( bufptr[fd] == (char) size)
	{
		return (1);
	}
	return (0);
