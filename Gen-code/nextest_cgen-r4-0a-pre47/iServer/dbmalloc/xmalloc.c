 ...
 */
#ifndef lint
static char rcs_hdr[] = "$Id: xmalloc.c,v 1.1 2002/07/04 12:53:10 agoyal Exp $";
#endif
...
char *
XtMalloc( unsigned int size )
{
	return( debug_XtMalloc((char *) NULL, 0, size) );
}

char *
debug_XtMalloc( CONST char *	file,
				int				line,
				unsigned int	size )
{
...
char *
debug_XtRealloc(	CONST char *	file,
					int				line,
					char *			ptr,
					unsigned int	size )
{
...
char *
debug_XtCalloc( CONST char *	file,
				int				line,
				unsigned int	num,
				unsigned int	size )
{
...
void
NeverCalledFunctionFromAnywhere(void)
{
	_XtHeapInit(NULL);
}
