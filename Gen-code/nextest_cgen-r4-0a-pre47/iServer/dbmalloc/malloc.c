...
#ifndef lint
static char rcs_hdr[] = "$Id: malloc.c,v 1.1 2002/07/04 12:53:10 agoyal Exp $";
#endif

#ifdef MALLOC_PREAMBLE
	int				malloc_preamble = 1;
#else
	int				malloc_preamble = 0;
#endif
...
int 			malloc_opts =	MOPT_MFILL  | MOPT_FFILL | MOPT_DFILL 	 |
								MOPT_CKDATA | MOPT_REUSE | MOPT_FREEMARK | MOPT_ZERO;
...
#ifdef MALLOC_PTHREAD
pthread_mutexattr_t	malloc_mutex_attr;
...
static VOIDTYPE malloc_dump_info_block(	CONST struct mlist *mptr, int id );
...
dbmalloc_init(void)
{
		...
		pthread_mutexattr_settype(	&malloc_mutex_attr,
									PTHREAD_MUTEX_DEFAULT );

		pthread_mutex_init( &malloc_mutex,
							&malloc_mutex_attr );
			...
			SIZETYPE size )
{
	...
	if (ptr == NULL)
	{
		...
		DEBUG2(10, "sbrk() returned 0x%lx, skipping %d bytes to align", cptr, fill);
	...
	if ((type == M_T_ALIGNED) && ((((SIZETYPE) (ptr->data)) & (malloc_align - 1)) != 0))
	{
...
malloc_split( struct mlist *ptr )
{
	...
	if (rest > sizeof(struct mlist))
	{
		...
		if (((ptr->flag & M_INUSE) != 0) || ((ptr->flag & M_FILLED) == 0))
		{
			...
			FILLDATA(tptr, FILL_SPLIT, (SIZETYPE) 0, (struct mlist *) NULL);
		}
 ...
 */
VOIDTYPE
malloc_fatal(	CONST char *funcname,
				CONST char *file,
				int line,
 ...
 */
VOIDTYPE
malloc_warning(	CONST char *funcname,
				CONST char *file,
				int line,
				CONST struct mlist *mptr )
{
 ...
 */
static VOIDTYPE
malloc_dump_info_block(	CONST struct mlist *mptr,
						int id )
{
	char errbuf[ERRBUFSIZE];
	CONST char *funcname;
	...
	{
		if ((mptr == NULL) || ((mptr == malloc_end) && (GETTYPE(mptr) == M_T_SPLIT)))
		{
	...
	case M_HANDLE_CORE:
		{
			...
			if ((pid = fork()) == 0)
			{
				VOIDCAST write(malloc_errfd, "Child dumping core\n", (WRTSIZE) 19);
				VOIDCAST signal(ABORT_SIGNAL, SIG_DFL);
			...
			if (access("core", 0) == 0)
			{
				...
				VOIDCAST tostring(filenam + 5, (ULONG) getpid(), 5, B_DEC, '0');

				filenam[10] = '.';

				VOIDCAST tostring(filenam + 11, (ULONG) corecnt++, 3, B_DEC, '0');
...
}		// end of malloc_freeseg(...)

CONST char *
MallocFuncName( CONST struct mlist *mptr )
{
	CONST char *rtn;
...
}		// end of MallocFuncName(...)

CONST char *
FreeFuncName( CONST struct mlist *mptr )
{
	CONST char *rtn;
 ...
 * Add
 *
 * Revision 1.10  2001/09/18 19:00:01  sturt
 * Update malloc.c to define global malloc_preamble and set it to zero
 * if MALLOC_PREAMBLE is not defined during configuration.
 *
 * Revision 1.9  2001/09/17 22:18:03  sturt
 * Remove mutex locks in malloc_freeseg()
 *
 * Revision 1.8  2001/09/17 22:08:01  sturt
 * Changes to mutex locking strategy to improve performance.
 *
 * Revision 1.7  2001/09/17 20:07:54  sturt
 * Move setting of malloc_preamble to 0 after initialization of malloc_mutex.
 *
 * Revision 1.6  2001/09/17 19:48:39  sturt
 * Small update.
 *
 * Revision 1.5  2001/09/17 19:48:01  sturt
 * Add new function dbmalloc_init() to do initializing if needed.
 *
 * Revision 1.4  2001/09/17 19:09:49  sturt
 * Whoops! dbmalloc_mutex_inited should be 0 to start with.
 *
 * Revision 1.3  2001/09/17 19:06:22  sturt
 * Make dbmalloc_mutex_inited a global.
 *
 ...
 *
 * Revision 1.6  2001/09/15 18:31:23  sturt
 * Add pthread mutex locking of malloc_mutex for solaris systems.
 *
 * Revision 1.5  2001/09/15 17:24:57  sturt
 * Start adding pthread mutex stuff
 *
 * Revision 1.4  2001/09/15 16:35:54  sturt
 * Reformating ...
 *
 ...
 * main().
 *
 * Revision 1.2  2001/09/14 16:00:25  sturt
 * Reformat malloc.c
 *
