...
#ifndef lint
static char rcs_header[] = "$Id: calloc.c,v 1.1 2002/07/04 12:53:10 agoyal Exp $";
#endif
 ...
 */
DATATYPE *
calloc(	SIZETYPE nelem,
		SIZETYPE elsize )
{
	return(	debug_calloc(	(char *) NULL,
							(int) -1,
							nelem,
							elsize ) );
}
 ...
 *
 * Purpose:
 *		to free an area allocated by calloc (actually frees any 
 *      allocated area)
 *
...
cfree(	DATATYPE *cptr )
{
	debug_cfree(	(CONST char *) NULL,
					(int) -1,
					cptr );
}

FREETYPE
debug_cfree(	CONST char *file,
				int line,
				DATATYPE *cptr )
{
	static IDTYPE call_counter;
 ...
 * Add
 *
 * Revision 1.2  2001/09/17 22:08:01  sturt
