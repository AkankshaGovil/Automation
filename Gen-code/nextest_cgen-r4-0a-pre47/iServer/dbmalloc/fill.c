			...
			int				showerrors )
{
	...
	else if (((ptr->flag & M_INUSE) != 0) || ((ptr->flag & M_FILLED) == 0))
	{
 ...
 */
void
FillData(	register struct mlist *	ptr,
			int						alloctype,
			SIZETYPE				start,
			struct mlist *			nextptr )
{
	...
	else
	{
		...
		{
			DataMS(ptr->data + ptr->r_size, M_FILL, (MEMSIZE) (limit - ptr->r_size));
		}
