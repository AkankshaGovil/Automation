...
#ifndef lint
static char rcs_hdr[] = "$Id: dump.c,v 1.1 2002/07/04 12:53:10 agoyal Exp $";
#endif
					...
					IDTYPE	histid2 )
{
	...
	for (ptr = &malloc_start; ptr; ptr = ptr->next)
	{
		...
		//
				 ...
				 && ((ptr->hist_id < histid1)
					 || (ptr->hist_id >= histid2) || ((ptr->flag & M_MARKED) != 0)))
		{
			continue;
		}
		...
		if (detail != DETAIL_NONE)
		{
			VOIDCAST tostring(buffer, (ULONG) ptr, 8, B_HEX, '0');
			VOIDCAST tostring(buffer + 9, (ULONG) ptr->next, 8, B_HEX, '0');
			VOIDCAST tostring(buffer + 18, (ULONG) ptr->prev, 8, B_HEX, '0');
			VOIDCAST tostring(buffer + 27, (ULONG) ptr->freenext, 8, B_HEX, '0');
			VOIDCAST tostring(buffer + 36, (ULONG) ptr->freeprev, 8, B_HEX, '0');
			VOIDCAST tostring(buffer + 45, (ULONG) ptr->flag, 10, B_HEX, '0');
			VOIDCAST tostring(buffer + 56, (ULONG) ptr->s.size, 8, B_DEC, ' ');
			VOIDCAST tostring(buffer + 65, (ULONG) ptr->s.size, 8, B_HEX, '0');
			buffer[64] = '(';
			buffer[74] = ')';
		}
		...
		VOIDCAST tostring(buffer + detail, (ULONG) ptr->data, 8, B_HEX, '0');
		...
		if ((ptr->file != NULL) && (ptr->file[0] != EOS))
		{
			...
			VOIDCAST tostring(buffer + detail + 30, (ULONG) ptr->line, 7, B_DEC, ' ');
		}
		else
		{
		...
		VOIDCAST tostring(buffer + detail + 53, (ULONG) ptr->r_size, 7, B_DEC, ' ');
