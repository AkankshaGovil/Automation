...
#ifndef lint
static char rcs_hdr[] = "$Id: mallopt.c,v 1.1 2002/07/04 12:53:10 agoyal Exp $";
#endif
			...
			union dbmalloptarg *	value )
{
	...
	case MALLOC_ERRFILE:
		...
		if (strcmp(value->str, "-") != 0)
		{
			...
			if (i == -1)
			{
				VOIDCAST write(2, "Unable to open malloc error file: ", (WRTSIZE) 34);
				for (s = value->str; *s; s++)
				{
					// do nothing 
					;
				}
				VOIDCAST write(2, value->str, (WRTSIZE) (s - (value->str)));
				VOIDCAST write(2, "\n", (WRTSIZE) 1);
			}
			else
			{
	...
	case MALLOC_FILLAREA:
		...
		case 3:
		default:
			newflag = MOPT_MFILL | MOPT_FFILL | MOPT_DFILL;
			break;
		}
		turnon = value->i;
