...
Mkfifo(const char *pathname, mode_t mode)
{
	...
	if ((retval = mkfifo(pathname, mode)) < 0)
		err_sys("mkfifo failed for %s", pathname);
	return(retval);
}
...
Free(void *ptr)
{
	if (ptr != 0) {
		...
		err_warn("Warning: free called with NULL argument \n");	
	}
}

// Our system function returns the return value of the called command
// from -128 to 127
// -128 indicates that it was an abnormal termination
int
System(const char *cmdstr)
{
	...
	if (WIFEXITED(status)) {
		err_warn("normal termination, exit status = %d\n", WEXITSTATUS(status));
		rval = (WEXITSTATUS(status)) - ( ((WEXITSTATUS(status)) & 0x80) ? 0x100 : 0 );
	}
	else if (WIFSIGNALED(status)) {
		err_msg("abnormal termination, signal number = %d%s\n", 
			WTERMSIG(status), WCOREDUMP(status) ? " (core file generated)" : "");
		rval = -128;		
	}
	else if (WIFSTOPPED(status)) {
		err_msg("child stopped. signal number = %d\n", WSTOPSIG(status));
		rval = -128;
	}
