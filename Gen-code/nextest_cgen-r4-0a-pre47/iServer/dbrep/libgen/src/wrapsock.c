...
int
RSConnect(int family, int type, int protocol, 
	const struct sockaddr *sa, socklen_t salen, int attempts, int ms)
{
	int i, fd = -1; 
...
//	fd = Socket(family, type, protocol);
	for (i = 0; i < attempts; i++) {
		...
		if (connect(fd, sa, salen) < 0) 
			err_ret("connect: error %d of %d attempt", i+1, attempts);
		else
			break; 	/* Success */

		msleep(ms);
		/* The old sock is useless get a new socket now */
		close(fd);
	//	fd = Socket(family, type, protocol);
	}

	if (i == attempts )  {
		err_sys("connect: could not connect server in %d attempts.\n", attempts);
	}
