...
SIP_S8bit sip_hashInit
	...
	(pHash, fpHashFunc, fpCompareFunc, fpElemFreeFunc, \
		fpKeyFreeFunc, numBuckets, maxElements,pErr)
	SipHash *pHash;
...
{
	SIP_U32bit i;
	SIPDEBUGFN((SIP_S8bit*)"SIP_DEBUG - Entering HashInit Function");
	...
	pHash->ppHashChains = (SipHashElement **) \
		fast_memget(NON_SPECIFIC_MEM_ID, sizeof(SipHashElement*)*numBuckets,\
			SIP_NULL);
	...
	pHash->pBucketMutex = (synch_id_t *) \
		fast_memget(NON_SPECIFIC_MEM_ID, sizeof(synch_id_t)*numBuckets, \
		SIP_NULL);
	if (pHash->pBucketMutex == NULL)
	{
		fast_memfree(NON_SPECIFIC_MEM_ID, (SIP_Pvoid )(pHash->ppHashChains), \
			SIP_NULL);
		*pErr = E_NO_MEM;
		return SipFail;
	}
	...
	SIPDEBUGFN((SIP_S8bit*)"SIP_DEBUG - Exiting HashInit Function");
	return SipSuccess;
}
...
#endif
{
	...
	fast_memfree(NON_SPECIFIC_MEM_ID, (SIP_Pvoid )(pHash->ppHashChains), pErr);
#ifdef SIP_THREAD_SAFE
	for(i=0; i<pHash->numberOfBuckets; i++)
	{
		 fast_free_synch(&(pHash->pBucketMutex[i]));
	}
	fast_memfree(NON_SPECIFIC_MEM_ID, (SIP_Pvoid )(pHash->pBucketMutex), pErr);
#endif
	return SipSuccess;
}
...
#endif
{
	...
	SIPDEBUGFN((SIP_S8bit*)"SIP_DEBUG - Entering HashAdd Function");
	...
	pNewElement = (SipHashElement *)\
		fast_memget(NON_SPECIFIC_MEM_ID, sizeof(SipHashElement), SIP_NULL);
	...
	SIPDEBUGFN((SIP_S8bit*)"SIP_DEBUG - Exiting HashAdd Function");
	return SipSuccess;
}
...
#endif
{
	...
	SIPDEBUGFN((SIP_S8bit*)"SIP_DEBUG - Entering HashFetch Function");
	...
	fast_unlock_synch(0, &(pHash->pBucketMutex[bucket]));
#endif
	SIPDEBUGFN((SIP_S8bit*)"SIP_DEBUG - Exiting HashFetch Function");
...
#endif
{
	...
	SIPDEBUGFN((SIP_S8bit*)"SIP_DEBUG - Entering HashRemove Function");
	...
	if (HSS_CHECKREF((*ppIterator)->dRefCount))
	{
		...
		*ppIterator = (*ppIterator)->pNext;
		...
		fast_memfree(NON_SPECIFIC_MEM_ID,(SIP_Pvoid )pTempElement,SIP_NULL);
		pHash->numberOfElements--;
	}
	else
	{
	...
	fast_unlock_synch(0, &(pHash->pBucketMutex[bucket]));
#endif
	SIPDEBUGFN((SIP_S8bit*)"SIP_DEBUG - Exiting HashRemove Function");
	return SipSuccess;
}
...
#endif
{
	...
	SIPDEBUGFN((SIP_S8bit*)"SIP_DEBUG - Entering HashRelease Function");
	...
	SIPDEBUGFN((SIP_S8bit*)"SIP_DEBUG - Exiting HashRelease Function");
}
...
{
	SIP_U32bit index;
	SIPDEBUGFN((SIP_S8bit*)"SIP_DEBUG - Entering HashForeach Function");
	...
	for(index = 0; index < pHash->numberOfBuckets; index++)
	{
		...
		while(pIterator != SIP_NULL)
		{
			...
			{
				SIPDEBUGFN((SIP_S8bit*)"SIP_DEBUG - Exiting "
	...
	SIPDEBUGFN((SIP_S8bit*)"SIP_DEBUG - Exiting HashForeach Function");
}
...
#endif
{
	...
	SIPDEBUGFN((SIP_S8bit*)"SIP_DEBUG - Entering HashNext Function");
			...
			(pIterator->pCurrentElement->dRemove == SipSuccess))
		{
			...
			if(*ppElement != SIP_NULL)
			{
				...
				*ppElement = (*ppElement)->pNext;
				...
				HSS_UNLOCKREF(pTempElement->dRefCount);
				HSS_DELETEREF(pTempElement->dRefCount);
				fast_memfree(NON_SPECIFIC_MEM_ID,(SIP_Pvoid )pTempElement, \
					SIP_NULL);
				pHash->numberOfElements--;
			}
		}
	...
	fast_unlock_synch(0, &(pHash->\
		pBucketMutex[pIterator->currentBucket]));
#endif
	SIPDEBUGFN((SIP_S8bit*)"SIP_DEBUG - Exiting HashNext Function");
}
...
#endif
{
	SIPDEBUGFN((SIP_S8bit*)"SIP_DEBUG - Entering "
		"HashInitIterator Function");
...
#endif
{
	SIP_U32bit h = 0, g;
	char *name;
	SIPDEBUGFN((SIP_S8bit*)"SIP_DEBUG - Entering ElfHash Function");
	if (pName == NULL)
		return (SIP_U32bit)0;
	...
	SIPDEBUGFN((SIP_S8bit*)"SIP_DEBUG - Exiting ElfHash Function");
	return h;
}
