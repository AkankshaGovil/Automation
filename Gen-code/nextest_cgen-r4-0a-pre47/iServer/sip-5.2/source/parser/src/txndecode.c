 ...
 **
 ** DESCRIPTION:
 **             This file contains the logic for primary txn layer handling.
 **             Basically the first level functions invoked by an applicn.
 **
...
#endif
{
	...
	if (dDirectOption==SipFail)
	{
		...
		if (sip_formMessage(pMessage,pSipOptions,pBuffer,&dLength,pError) \
				==	SipFail)
		{
			fast_memfree(TXN_MEM_ID,pBuffer,NULL);
			return SipFail;
		}
	}
	else
	...
	if (dBypassEnable==SipTxnByPass)
	{
		...
		if (dResult!=SipSuccess)
		{
			...
			if (!dDirectOption)
			fast_memfree(TXN_MEM_ID,pBuffer,pError);
			return SipFail;
		}
	...
	if (sip_isTxnAnRPR(&txnInformn)==SipSuccess)
	{
		...
		if (dResult!=SipSuccess)
		{
			...
			/*Free the buffer in TxnInformn*/
			if (!dDirectOption)
				fast_memfree(TXN_MEM_ID,pBuffer,&intError);
			return SipFail;
		}
	}		
	else
	...
	if (txnInformn.dMesgSentAtLeastOnce==SipFail)
	{
		...
		if (dResult!=SipSuccess)
		{
			if (txnInformn.pMsgBuffer!=SIP_NULL)
			{
				/*Free the buffer in TxnInformn*/
				if (!dDirectOption)
				fast_memfree(TXN_MEM_ID,txnInformn.pMsgBuffer,pError);
			}
			return SipFail;
		}
	...
	{
		/*
		 * Free the EventContext for RPR scenarios (ProxyType of
		 * transactions alone)
		 */
		if (sip_isTxnAnRPR(&txnInformn)==SipSuccess && \
...
#endif
{
	...
	if (dResult!=SipSuccess)
	{
		if (*pError==E_TXN_INV_MSG)
		{
			/*In this case free up the message, since this has 
				hapnd because of the mandatory hdrs check*/
			sip_freeSipMessage(*ppDecodedMessage);
			*ppDecodedMessage = SIP_NULL;
		}
		/*Handle failure  here*/
		return SipFail;
	}
...
#endif
{
	...
	*pErr =  E_NO_ERROR;
	...
	if((pGeneratedKey->dTagCheck) && (pStoredKey->dTagCheck))
	{
		...
		{
			if(strcmp(pGeneratedKey->pToTag,pStoredKey->pToTag) != 0)
			return SipFail;
		}
		else
		{
	...
	{
		if(strcmp(pGeneratedKey->pFromTag,pStoredKey->pFromTag) != 0)
		return SipFail;
	}
	else
	{
	...
	if(strcmp(pGeneratedKey->pMethod,"ACK") != 0)
	{
		dIsItAnAck=SipFail;
		if(strcmp(pGeneratedKey->pMethod,pStoredKey->pMethod) != 0)
			return SipFail;
	}
	else
	{
		 ...
		 */
		if (strcmp(pStoredKey->pMethod, "INVITE") != 0)
			return SipFail;
	}
	
	if(strcmp(pGeneratedKey->pCallid,pStoredKey->pCallid) != 0)
		return SipFail;
	...
	if (dIsItAnAck == SipFail)
	{
		...
		{	
			...
			{
				if (strcmp(pGeneratedKey->pViaBranch,\
	...
	*/
	...
	{
		if(strcmp(pGeneratedKey->pRackHdr->pMethod, \
			pStoredKey->pRackHdr->pMethod) != 0)
			return SipFail;

		if((pGeneratedKey->pRackHdr->dRespNum !=  \
			pStoredKey->pRackHdr->dRespNum) ||    \
			(pGeneratedKey->pRackHdr->dCseqNum != \
			pStoredKey->pRackHdr->dCseqNum) )
			return SipFail;
	}
...
#endif
{
	...
	pKey->pCallid= (SIP_S8bit *)STRDUPNETWORK(pGenHdr->pCallidHdr->pValue);
	...
	{
		pKey->pMethod = (SIP_S8bit *)STRDUPNETWORK(pGenHdr->pCseqHdr->pMethod);
		if(pKey->pMethod == SIP_NULL)
		{
			*pErr = E_NO_MEM;
		 	return SipFail;
		...
		pKey->dRespCodeNum = pMessage->u.pResponse->pStatusLine->dCodeNum;
	}

	pKey->dCSeq = pGenHdr->pCseqHdr->dSeqNum;
	...
	if( dType == SipMessageResponse )
	{
		...
		if(pRespHdr->pRSeqHdr)
		{
			...
			if((dCodeNum < 200)&&(dCodeNum > 100))
				pKey->dRseq = pRespHdr->pRSeqHdr->dRespNum;
		}
	}

	/*Get the From Tag*/
	if(sip_listGetAt(&(pGenHdr->pFromHeader->slTag), 0, \
			(SIP_Pvoid *) &pTempStr , pErr) != SipFail)
	{
	...
	/*Get the To Tag*/
	if(sip_listGetAt(&(pGenHdr->pToHdr->slTag), 0, \
			(SIP_Pvoid *) &pTempStr , pErr) != SipFail)
	{
	...
	if( dType == SipMessageRequest )
	{
		...
		{
			if(sip_rpr_initSipRAckHeader(&((pKey)->pRackHdr),\
				 pErr)==SipFail)
                return SipFail;
			pKey->pRackHdr->dRespNum = pRackHdr->dRespNum;
			pKey->pRackHdr->dCseqNum = pRackHdr->dCseqNum;
			pKey->pRackHdr->pMethod = (SIP_S8bit *)STRDUPNETWORK(pRackHdr->\
							pMethod);
		}
		else
		{
