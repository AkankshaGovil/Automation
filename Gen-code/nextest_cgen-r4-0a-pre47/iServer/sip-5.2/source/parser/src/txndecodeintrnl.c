 ...
 ** ----        ----                    ---------              --------
 **
 ** 9/02/2002  R Kamath 	                               Initial Version
 **
 **
...
#endif
{
	...
	if  (pTxnInfo->dMesgType==SipMessageResponse)
	{
		...
		dCodeNum = pTxnInfo->pSipMessage->u.pResponse->pStatusLine->dCodeNum;
...
#endif
{
	...
	if (s->dType == SipMessageRequest)
	{
		...
		if((s->pGeneralHdr->pToHdr == SIP_NULL))
			MissingHdrCount++;
		if(strcmp(s->u.pRequest->pRequestLine->pMethod,"PRACK")==0)
		{
			/*
...
#endif
{
    ...
    {
		pMethod = pMessage->u.pRequest->pRequestLine->pMethod;
		if ( strcmp(pMethod,pMessage->pGeneralHdr->pCseqHdr->pMethod) != 0 )
		{
	...
	{
		pMethod = pMessage->pGeneralHdr->pCseqHdr->pMethod;
	}
	...
	if(strlen(pMethod) <= SIP_TXN_MAX_METHOD_SIZE)
	{
		strcpy(pTxnInfo->methodName,pMethod);
	}
	else
	{
		strcpy(pTxnInfo->methodName,"UNKNOWN");
	}
	...
	{
		dCodenum=pMessage->u.pResponse->pStatusLine->dCodeNum;
...
#endif
{
	...
	if(pTxnInfo->dMesgType == SipMessageResponse)
	{
		...
		if ( dTempCodeNum < NUM_STAT_RESPONSECODE )
		{
			...
			{				
				...
				HSS_INCREF(pMesg->dRefCount);

				sip_indicateResponseHandled(pMesg, \
					SipCallbkForResponseSent, dTempCodeNum,\
					glbSipParserResponseCodeParserStats[dTempCodeNum]);
			}	
	...
	else if(pTxnInfo->dMesgType == SipMessageRequest)
	{
		INC_API_REQ_SENT
#ifdef SIP_MIB
		pTempMethod = pMesg->u.pRequest->pRequestLine->pMethod;
	...
	sip_trace(SIP_Detailed,SIP_Outgoing,pBuffer);
#endif
...
#endif
{
	...
	else
	{
		...
		dResult= RELEASE_TXN((SIP_Pvoid)pTxnKey,(SIP_Pvoid *)&pTempKey, \
					(SIP_Pvoid *)(&pTxnBuffer),pContextData,dRemove,pError);
		if (dResult!=SipSuccess || retVal == SipFail)
		{
			/*Handle failure here*/
			return SipFail;
		}
...
#endif
{
	...
	SipBool	dResult;
	SipTxnInformn *pTempInfo=SIP_NULL;
#ifdef SIP_TRACE
...
#ifdef SIP_TRACE
	strncpy (mesg, (SIP_S8bit *)pBuffer, 20);
	mesg[20]='\0';
	HSS_SPRINTF (traceBuf,"Message Remote Retranmsmitted \n" \
	 					"(1st 20 bytes) : \"%s\" ",mesg);
...
#ifdef SIP_MIB
	pTimeoutValues->dT2 = SIP_T2;
	if(pTxnInfo->dMesgType == SipMessageRequest)
	{
		SIP_S8bit* pTempMethod;
		SipConfigStats *pConfig;
		SipError dErr;
		pTempMethod = STRDUP(pTxnInfo->methodName);
		if( SipSuccess == __sip_getConfigStatsForMethod\
						(pTempMethod,&pConfig,&dErr))
		{
			/* check if the timer values are configured*/
			if(pConfig != SIP_NULL)
			{
				pTimeoutValues->dT1 = pConfig->dRetransT1;
			}
		}
		fast_memfree(0,pTempMethod,SIP_NULL);
	}
	else
	{
		SipConfigStats *pConfig;
...
#endif
{
	...
	if (pTxnBuffer->pAddr!=SIP_NULL)
	{
		if (pTxnBuffer->pAddr->pHost!=SIP_NULL)
			fast_memfree(0,pTxnBuffer->pAddr->pHost,SIP_NULL);
		fast_memfree(0,pTxnBuffer->pAddr,SIP_NULL);
	}
				...
				else
				{
					...
					||(pTxnBuffer->dTxnState==SipTxnInvClientProceedingState))
					{
						...
						*pDecodeResult=SipTxnNonIgnorable;
						...
						dResult=sip_listGetAt \
								(&(pMesg->pGeneralHdr->pToHdr->slTag), \
								0, (SIP_Pvoid *) &pTempStr , pError);
						if (dResult==SipSuccess)
						{
							/*
							...
							if (pToBeFreed!=SIP_NULL)
							{
								 ...
								 */
								SipError	tempError;
								fast_memfree(TXN_MEM_ID,pToBeFreed,&tempError);
							}
			...
			||(pTxnBuffer->dTxnState==SipTxnInvClientProceedingState))
		{
			*pDecodeResult=SipTxnNonIgnorable;
			...
			else
			{
				 ...
				 * will be tolerated
				 */
				dResult=sip_listGetAt(&(pMesg->pGeneralHdr->pToHdr->slTag), \
						0, (SIP_Pvoid *) &pTempStr , pError);
				if (dResult==SipSuccess)
				{
					pToBeFreed=pTxnBuffer->pTxnKey->pToTag;
					...
					if (pTxnBuffer->dTxnState==SipTxnInvServerCompletedState)
					{
						...
						if(pTxnBuffer->dRespCode/100 != 2)
						{
							if ( (pTxnBuffer->pTxnKey->pViaBranch != \
								...
								SIP_NULL) )
							{
								if(strcmp(pTxnBuffer->pTxnKey->pViaBranch,\
...
#endif
{
	...
	if (pTxnBuffer->dTxnState==SipTxnServerBypassState)
	{
		...
		if (dResult!=SipSuccess)
		{
			...
			tempError = E_NO_ERROR;
			pTxnBuffer->pBuffer = SIP_NULL;
			sip_releaseTxnNFreeKey(pTxnBuffer,pTxnKey, \
											   pTxnInfo,&tempError);
			return SipFail;
		}
		pTxnInfo->dMesgSentAtLeastOnce=SipSuccess;
			...
			default:
				{
					...
					if (pTxnBuffer->dTxnState!=SipTxnInvServerConfirmedState)
					{
						 ...
						 */
						dResult=sip_listGetAt(\
							&(pMesg->pGeneralHdr->pToHdr->slTag), 0,\
							(SIP_Pvoid *) &pTempStr , pError);
						if (dResult==SipSuccess)
						{
							pToBeFreed=pTxnBuffer->pTxnKey->pToTag;
							if (pToBeFreed!=SIP_NULL)
...
#endif
{
	...
	if(sip_listSizeOf(pslRPRTxnKeyQ, &count, pError) !=SipFail)
	{
		while(index < count )
		{
			if(sip_listGetAt(pslRPRTxnKeyQ, index, \
...
#endif
{
	...
	if(pTxnBuffer->pslRPRTxnKeyQ == SIP_NULL)
	{
		...
		sip_listInit(((pTxnBuffer)->pslRPRTxnKeyQ ),__sip_freeSipTxnKey,pError);
	...
	else
	{
		if(sip_listSizeOf(pTxnBuffer->pslRPRTxnKeyQ, &count, pError)!=SipFail)
		{
			while(index < count )
			{
				if(sip_listGetAt(pTxnBuffer->pslRPRTxnKeyQ, index, \
...
#endif
{
	...
	if(dResult != SipSuccess)
	{
		sip_releaseTxnNFreeKey(pTxnBuffer,pClonedKey,pTxnInfo,pError);
...
#endif
{
	...
	sip_initSipTxnKey(&pClonedKey,pError);
	sip_cloneSipTxnKey(pClonedKey,pTxnKey,pError);
	...
	pClonedKey->dCSeq = pClonedKey->pRackHdr->dCseqNum;
	pTempMethod = pClonedKey->pMethod;
	pClonedKey->pMethod = pClonedKey->pRackHdr->pMethod;
	pClonedKey->pRackHdr->pMethod = pTempMethod;
	pClonedKey->dTxnClass=SipInviteClient;
	...
	pClonedKey->dRseq = pClonedKey->pRackHdr->dRespNum;
...
#endif
{
	...
	/*For Removing compiln problem*/
	pDecodeResult=SIP_NULL;
...
#endif
{
	...
	else
	{
		...
		{
			sip_releaseTxnNFreeKey(pTxnBuffer,pTxnKey,pTxnInfo,pError);
			*pError = E_TXN_INV_STATE;
			return SipFail;
		}
...
#endif
{
	...
	if(pTxnKey->dType == SipMessageResponse)
	{
		INC_API_RESP_SENT
#ifdef SIP_MIB
		dTempCodeNum = pTxnKey->dRespCodeNum;
		if ( dTempCodeNum >= 100 && dTempCodeNum < 200)
		...
		if ( dTempCodeNum < NUM_STAT_RESPONSECODE )
		{
			INC_API_RESPCODE_PARSED_RETRY(dTempCodeNum)
		}
#endif
	}
...
#endif
{
	...
	{
		if (strcmp(pTransp2->pHost,pTransp1->pHost))
			return SipFail;
	}
...
#endif
#ifdef SIP_TRACE
			strncpy (msg, (SIP_S8bit *)pTxnBuffer->pBuffer, 20);
			msg[20]='\0';
			sip_trace(SIP_Brief,SIP_Outgoing,(SIP_S8bit *) "Message sent out.");
			sip_trace(SIP_Detailed,SIP_Outgoing,(SIP_S8bit *)\
			                                    pTxnBuffer->pBuffer);
	...
	{ /*
		...
		*/
		...
		if ((dWhichTimer==SipTimerC)&&(pTxnBuffer->dType==SipProxyTypeTxn))
		{
			...
			dRetVal=sip_startTimerCr(pTxnBuffer,pTxnKey,pTempEventContext,\
									pTxnTimeoutData,&err);
		...
		{
			RELEASE_TXN(pTxnKey,(SIP_Pvoid*)&pTxnOutKey,\
...
#endif
{
	...
	sip_initSipTxnKey(&pClonedKey,pErr);
	sip_cloneSipTxnKey(pClonedKey,pTxnKey,pErr);
...
#endif
{
	...
	/*The below instrn is strictly for ensuring compiln without errors*/
	pKey=SIP_NULL;
