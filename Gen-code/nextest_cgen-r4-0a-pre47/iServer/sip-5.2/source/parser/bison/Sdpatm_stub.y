...
%token HEXVAL ATMNETTYPE ATMADDRTYPE URICNOCOLON URICOLON URI
...
%token ZEQ ALPHASAFE IPV6ADDR BYTESTR TOKEN
%token DEFAULT
%start line
%%
line:protoversion 
...
sessionandnet: xsessionid xsessionver atmnettype atmaddrtype
	{
			...
			if ( (tempid[1] == 'X' || tempid[1] == 'x') ||
			     (tempver[1] == 'X' || tempver[1] == 'x') )
			{
				if ( strcasecmp(tempnet,"IN") == 0 )
				{
					*(((SipSdpParserParam *) pSdpParserParam)->pError) =\
					E_PARSER_ERROR;
					free(tempid);
			...
			if ( (strcasecmp(tempnet,"IN")==0 && strncasecmp(tempaddr,"IP",2)!=0) || 
			     (strcasecmp(tempnet,"IN")!=0 && strncasecmp(tempaddr,"IP",2)==0) )
			{
				*(((SipSdpParserParam *) pSdpParserParam)->pError) =\
				E_PARSER_ERROR;
				free(tempid);
...
ctypes:NETTYPE addrtype connectaddr
	...
	|netatmtype atmaddrtype atmconnectaddr
	{
#ifndef SIP_ATM
		*(((SipSdpParserParam *) pSdpParserParam)->pError) =\
		E_PARSER_ERROR;
