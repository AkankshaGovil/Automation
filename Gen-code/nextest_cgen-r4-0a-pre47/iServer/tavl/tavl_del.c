 ...
 */
void *
tavl_delete (TAVL_treeptr tree, void *key,TAVL_nodeptr ret)
{
    char rb, deltaht;
    int side;
    int  found = deltaht = 0;
...
#define POP_PATH(x)     (x = (--next)->side, (next->p))
    ...
    while (p) {
        cmpval = (*tree->cmp)(key,(p->dataptr),(tree->cmpParam));
        if (cmpval > 0) {
            PUSH_PATH(p,RIGHT);
            p = Rightchild(p);
        }
    ...
    //Taking node data to return later.
    //memcpy(ret, q,sizeof(TAVL_NODE));
    ret->dataptr = q->dataptr;
