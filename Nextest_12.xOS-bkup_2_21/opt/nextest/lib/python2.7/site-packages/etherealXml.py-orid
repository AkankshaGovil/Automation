...
try:
    ...
    #ignoring exception as temporary fix on solaris
    pass
...
class PacketList:
    ...
    #PR 135088
    def get_items_show(self,string,items=None):
        ...
        for child in self.children:
            if child.show == string:
                items.append(child)
            child.get_items_show(string, items)
...
class Packet(ProtoTreeItem, PacketList):
    ...
    def getFieldFirstValue(self, name):
        """ return the value of the first field with name matching name
        """
        itemList = []
        self.get_items(name, itemList)
        try:
            return itemList[0].get_show()
        except:
            return None

    def getFieldFirstShowName(self, name):
        """ return the ShowName of the first field with name matching name
        """
        itemList = []
        self.get_items(name, itemList)
        try:
            return itemList[0].get_showname()
        except:
            return None

    def getFieldAllValues(self, name):
        """ return values of all fields matching name
        """
        itemList = []
        itemValues = []
        self.get_items(name, itemList)
        for item in itemList:
            itemValues.append(item.get_show())

        return itemValues

    #18523
    def getFieldAllNames(self, name):
        """ return names of all fields matching name
        """
        itemList = []
        itemNames = []
        self.get_items(name, itemList)
        for item in itemList:
            itemNames.append(item.get_showname())

        return itemNames
    
    #PR 135088
    def getFieldFindShow(self,string):
        itemList = []
        self.get_items_show(string, itemList)
        for item in itemList:
            if (item.get_show()==string):
	        return True 

        return None 
    
    
    def getFieldFindShowValue(self,string):
        itemList = []
        self.get_items_show(string, itemList)
        for item in itemList:
            if (item.get_show()==string):
    ...
    #PR 135088
    def getFieldFirstShow(self,string):
        itemList = []
        self.get_items_show(string, itemList)
	#$print itemList
        try:
    ...
    def assertFields(self, fields,multInsField=False):
        """
          assert the packets for fields specified in the fields list with specified values
          returns a result code and a result String. If all assertion pass the result code
          will be set to 1, if one check fails the result will be 0 and the result string will
          contain a error message, no further tests will be done
          # 56430
          multInsField: This parameter specifies whether the field would be present multiple times
          in the packet. Default value is False.
        """

        frameNumber = self.getFieldFirstValue('frame.number')
        result = 1
        resultStr = 'Frame %s: Assert passed' % (frameNumber)
        #17439
        #Modified to capture the failure result for all the cases .
        errStr = ''

        for key in fields.keys():
            actualValue = self.getFieldAllValues(key)
            if (len(actualValue) > 1 ):
              # 56430 - Return false if multInsField parameter is set to False
              if multInsField:
                for i in range(0,len(actualValue)):
                  present = False
                  if actualValue[i] in fields[key]:
                    present = True
                    break
                if not present:
                  result = 0
                  errStr += '%s: got (%s), expected (%s)\n' % (key, actualValue, fields[key])
              else:
                result = 0
                errStr += 'multiple instances of field %s\n' % key

            elif (len(actualValue) < 1):
              result = 0
              errStr += 'no such field %s\n' % key

            elif (actualValue[0] not in fields[key]):
              result = 0
              errStr += '%s: got (%s), expected (%s)\n' % (key, actualValue, fields[key])
        #17439
        if (result == 0):
            resultStr = 'Frame %s error - %s' % (frameNumber,errStr)
