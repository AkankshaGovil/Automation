...
class Database(qm.extension.Extension):
    ...
    def ExpandIds(self, ids, Flag="OFF"):
        ...
        for id in tempids:
            ...
            # Or is this a test ID?
                ...
                # Yes.  Add it.
                test_ids[id] = None
                # If it is a Test ID, add the prerequisite also to the list of test ids
                # changes made for ticket #14090
                if Flag == 'ON':
                    testDesc = self.GetTest(id)
                    testprereq = testDesc.GetPrerequisites()
                    testprereqlist = testprereq.keys()
                    tempids.extend(testprereqlist)
                    #If it is a Test ID, add the xit also to the list of test ids
                    # unless it is a provision script, we wont add the xit script
                    if not id.endswith('.provision'):
                        continue
                    try:
                        # Substitute provision with xit and add it as a test case
                        splitid = (id.split('.')[0:-1])
                        splitid.append('xit')
                        xitscriptid = string.join(splitid,'.')
                        if self.HasTest(xitscriptid):
                            test_ids[xitscriptid] = None
                    except:
                        continue
            else:
