...
class EnduranceEngine:
    """A 'EnduranceEngine' executes tests.

    A 'EnduranceEngine' object handles the execution of a collection
    of tests.
    ...
    def __init__(self,
        ...
        if expectations is not None:
            ...
            self.__expectations = {}
        # If duration is specified, run tests for that duration
	if duration is not None:
            self.__duration = duration
	    self.__timeExpired = False
	else:
	    self.__duration = None
            self.__timeExpired = True
    ...
    def startTimer(self):
        """Starts a thread to keep track of the execution time of endurance
        tests"""
        def timedOut():
            self.__timeExpired = True

        days, hours, min = self.__duration.split(':')
        timeout = 24*60*60*int(days) + 60*60*int(hours) + 60*int(min)
        t = threading.Timer(timeout, timedOut)
        t.start()
        return t

    def resetExecEngine(self):
        """Reshuffle the test ids.
        
        Reset all necessary data structures for execustion engine to
        re-run the tests.  Reset test status back to initial status.
	"""
        self.input_handler = {}
        self.__running = 0
        if self.__randomize:
            random.shuffle(self.__test_ids)
