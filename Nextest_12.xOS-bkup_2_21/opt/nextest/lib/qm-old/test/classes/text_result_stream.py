...
class TextResultStream(FileResultStream):
    ...
    def WriteResult(self, result):
        ...
        #print out any subresults. Beautiful recursion, allows results
        #to be embedded inside results to any level.
        try:
    ...
    def WriteAnnotation(self,key,value):
        """
           For displaying the annotations on the standard output
           Fix for ticket 12367
        """
        if (key == "MSW FireWall"):
            self._DisplayHeading("TEST BED INFORMATION")
            self.file.write(key+"    :%s\n" %value )
        if ((key == "Date(MM/DD/YYY)") or (key == "Time (24H:MM)") or  
           (key == "MSW Name") or (key == "Nextest Version") or 
           (key == "MSW IVersion") or (key == "Nextest Host") or 
           (key == "MSW OS")):
            self.file.write(key+"    :%s\n" %value )     
