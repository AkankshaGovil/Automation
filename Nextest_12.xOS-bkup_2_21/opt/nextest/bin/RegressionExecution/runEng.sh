...
suitefile=suites_nexunit$MajorVersion.txt
...
while true; do
        ...
        /opt/nextest/bin/qmtest run -c nextest.scm_configuration=ON -c nextest.trace_level=DEBUG -c nextest.disable_debugflags=OFF -c nextest.radius_verify=OFF -o results_$line.qmr eng_test.$line

        Result_File=results_nexunit_$line.txt

        sudo cp /tmp/nextest.log /var/temp/$line.log
        qmtest summarize -f full results_$line.qmr > $Result_File
        mv results_nexunit_$line.txt $resultDir
        count=`expr $count + 1`
