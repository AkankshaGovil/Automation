...
class Packet(UserDict.UserDict):
	...
	def __init__(self, code=0, id=None, secret="", authenticator=None, **attributes):
		...
		self.code=code
                # 32141 - Fixed a Bug
                # New ID should not be created for id=0
		if id != None:
			self.id=id
		else:
			self.id=CreateID()
	...
	def ReplyPacket(self):
		...
		header=struct.pack("!BBH", self.code, self.id, (20+len(attr)))
                ...
                #JS - Modified this as it was creating problem when integrated with Nextest
		#authenticator=md5.new(header[0:4] + self.authenticator
		#	+ attr + self.secret).digest()
                m = md5.new()
                m.update(header[0:4])
                m.update(self.authenticator)
                m.update(attr)
                m.update(self.secret)
                authenticator=m.digest()
